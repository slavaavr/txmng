...
func TestTxManager_PGX(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			runJob: func(txm txmng.TxManager, dbm txmng.DBManager[txmng.PGXDB]) (txmng.Result, error) {
				return txm.RunTx(txOpts, func(ctx txmng.Context) (txmng.Result, error) {
					db := dbm.GetDB(ctx)
		...
		},
		{
			...
			runJob: func(txm txmng.TxManager, dbm txmng.DBManager[txmng.PGXDB]) (txmng.Result, error) {
				return txm.RunNoTx(noTxOpts, func(ctx txmng.Context) (txmng.Result, error) {
					db := dbm.GetDB(ctx)
...
func TestTxManager_StdDB(t *testing.T) {
	ctx := t.Context()
	txOpts := txmng.TxOpts{
		Ctx:       ctx,
		Isolation: txmng.LevelSerializable,
...
func TestTxManager_PGX_RunParallel(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			runJob: func(txm txmng.TxManager, dbm txmng.DBManager[txmng.PGXDB]) {
				_, err := txm.RunTx(txOpts, func(ctx txmng.Context) (txmng.Result, error) {
					db := dbm.GetDB(ctx)
...
func TestTxManager_StdDB_RunParallel(t *testing.T) {
	ctx := t.Context()
	txOpts := txmng.TxOpts{
		Ctx:       ctx,
		Isolation: txmng.LevelSerializable,
